...
import com.thinkenterprise.controller.views.RouteView;
...
public class RouteController {
	...
	@ApiOperation(value="Get all Routes by RouteView", notes= "Read the list of routes from the repository", nickname="getAllRouteView", response=RouteView.class, responseContainer="List", code=200, tags={"Routes"}, produces="application/json")
	@ApiResponses(value = {@ApiResponse(code=204, message="empty list"), @ApiResponse(code=400, message="can't access routes ",response=Error.class)})
	@RequestMapping(params="view", method=RequestMethod.GET, produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<RouteView>> getAllRouteViews(@RequestParam("view") String view) {

		List<RouteView> responseEntity = mapperFacade.mapAsList(routeRepository.findAll(), RouteView.class);
		return new ResponseEntity<List<RouteView>>(responseEntity, HttpStatus.OK);
